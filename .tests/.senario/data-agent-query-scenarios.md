---
lastUpdate: 2026-03-24 13:42:57
author: Daniel Chung
version: 1.1.0
---
...
## 自動化測試結果 (v1 Baseline — 2026-03-24)
...
| 策略 | 總數 | 通過 | 失敗 | 通過率 |
|------|------|------|------|--------|
| template | 35 | 31 | 4 | 88.6% |
| small_llm | 25 | 19 | 6 | 76.0% |
| large_llm | 20 | 12 | 8 | 60.0% |

### 關鍵發現

1. **Template 策略最穩定** (88.6%) — 意圖定義清晰、nl_examples 覆蓋好
2. **Large LLM 策略最弱** (60.0%) — 跨模組複雜意圖 (mm_f01~f04) 的語義相似度過高，容易混淆
3. **模糊查詢門檻過低** — 目前 `match_threshold=0.5` 讓模糊查詢也能匹配到意圖 (score 0.55~0.74)
4. **Top-3 覆蓋率佳** — 正常查詢的 18 個失敗中，有 7 個 (39%) 在 Top-3 內，加入 Reranking 可改善

### 改善建議

| 優先級 | 建議 | 預估改善 |
|--------|------|----------|
| P0 | 為失敗意圖補充 nl_examples → re-sync Qdrant | +5~10% |
| P1 | 提高 match_threshold 到 0.6 以過濾模糊/錯誤查詢 | +7 ambiguous, +3 error |
| P2 | 為 mm_f01~f04 增加更具區別性的 description | +3~5% large_llm |
| P3 | 引入 Post-Retrieval Reranking (Top-3 → LLM 重排) | +5~7% (7 個 top-3 案例) |
